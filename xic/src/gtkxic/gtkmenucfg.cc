...
#define HANDLER (GtkUIManagerCallback)menu_handler
...
// This is the GtkUIManagerEntry structure used to generate new menus.
...
// namespace {
//     // Subclass GtkUIManagerEntry so can use constructor
//     //
//     struct if_entry : public GtkUIManagerEntry
//     {
//         if_entry()
//             {
//                 path = 0;
...
//                 item_type = 0;
//             }
//         if_entry(const char *p, const char *a, GtkUIManagerCallback cb,
...
{
    GtkUIManager *item_factory = gtkMenu()->itemFactory;
    if (!item_factory)
        return;
    // if_entry mi[50];
    ...
    //         i++;
    //     }
    //     GtkUIManagerEntry *menu = new GtkUIManagerEntry[j];
    //     memcpy(menu, mi, j*sizeof(GtkUIManagerEntry));
    ...
    //         XM()->OpenCellMenuList(), edmenu_proc, 0);
    //     if (popup) {
    //         gtk_widget_set_name(popup, "EditSubmenu");
    //         g_object_set_data(G_OBJECT(btn), "menu", popup);
    //         g_object_set_data(G_OBJECT(btn), "callb", (void*)edmenu_proc);
    ...
    //             gtk_widget_set_tooltip_text(btn, mbox->menu[fileMenuOpen].description);
    //         }
    //     }
    ...
    //         "/View/View");
    //     GtkWidget *popup = gtkMenu()->new_popup_menu(btn,
    //         XM()->ViewList(), vimenu_proc, 0);
    //     if (popup) {
    //         gtk_widget_set_name(popup, "ViewSubmenu");
    //         g_object_set_data(G_OBJECT(btn), "menu", popup);
    //         g_object_set_data(G_OBJECT(btn), "callb", (void*)vimenu_proc);
    ...
    //             gtk_widget_set_tooltip_text(btn, mbox->menu[viewMenuView].description);
    //         }
    //     }
...
gtkMenuConfig::instantiateTopButtonMenu()
{
    ...
    //             G_CALLBACK(top_btnmenu_callback), (void*)(long)i);
    //         if (ent->description) {
...
gtkMenuConfig::instantiateSideButtonMenus()
{
    ...
    //                 EditIf()->styleList(), stmenu_proc, 0);
    //             if (menu) {
    ...
    //                 Menu()->Select(button);
    //             g_signal_connect(G_OBJECT(button), "clicked",
    //                 G_CALLBACK(btnmenu_callback), (void*)(long)i);
    //         }
    //         if (ent->description) {
    //             gtk_widget_set_tooltip_text(button, ent->description);
    //         }
    ...
    //                 false);
    //             GtkWidget *menu = gtkMenu()->new_popup_menu(0,
    //                 ScedIf()->shapesList(), shmenu_proc, 0);
    //             if (menu) {
...
void
gtkMenuConfig::instantiateSubwMenus(int wnum, GtkUIManager *item_factory)
{
    // if (!item_factory)
    //     return;
    // if_entry mi[50];
    ...
    //         "/View/View");
    //     GtkWidget *popup = gtkMenu()->new_popup_menu(btn,
    //         XM()->ViewList(), vimenu_proc, (void*)(long)wnum);
    //     if (popup) {
    //         g_object_set_data(G_OBJECT(btn), "menu", popup);
    //         g_object_set_data(G_OBJECT(btn), "callb", (void*)vimenu_proc);
    //         g_object_set_data(G_OBJECT(btn), "data", (void*)(long)wnum);
...
{
    // if (wnum == 0) {
    //     GtkUIManager *ifact = gtkMenu()->itemFactory;
    //     if (!ifact)
    //         return;
...
//
// void
// gtkMenuConfig::make_entries(GtkUIManager *item_factory,
//     GtkUIManagerEntry *entries, int nitems, MenuEnt *menu, int menuitems)
// {
...
//             if (entries[i].callback == ()menu_handler &&
